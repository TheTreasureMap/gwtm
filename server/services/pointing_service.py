...
class PointingService:
    ...
    @staticmethod
    def validate_instrument_reference(
        pointing_data: "PointingCreate", instruments_dict: dict
    ) -> int:
    ...
    @staticmethod
    def check_duplicate_pointing(
        pointing: Pointing, existing_pointings: List[Pointing]
    ) -> bool:
        """Check if a pointing is a duplicate of existing pointings."""
        return pointing_crossmatch(pointing, existing_pointings)

    @staticmethod
    def create_pointing_from_schema(
        pointing_data: "PointingCreate", user_id: int, instrument_id: int
    ) -> Pointing:
    ...
    @staticmethod
    def handle_planned_pointing_update(
        pointing_data, user_id: int, db: Session
    ) -> Optional[Pointing]:
        ...
        planned_pointing = (
            db.query(Pointing)
            .filter(Pointing.id == pointing_id, Pointing.submitterid == user_id)
            .first()
        )
        ...
        if not planned_pointing:
            raise validation_exception(
                ...
                errors=[
                    f"Pointing with ID {pointing_id} not found or not owned by you"
                ],
            )

        if planned_pointing.status in [
            pointing_status_enum.completed,
            pointing_status_enum.cancelled,
        ]:
            raise validation_exception(
                message="Pointing validation error",
                errors=[
                    f"This pointing has already been {planned_pointing.status.name}"
                ],
            )
    ...
    @staticmethod
    def prepare_doi_creators(
        creators: Optional[List[dict]],
        doi_group_id: Optional[int],
        user: Users,
        db: Session,
    ) -> List[dict]:
    ...
    @staticmethod
    def create_doi_for_pointings(
        pointings: List[Pointing], graceid: str, creators: List[dict], db: Session
        ...
        insts = (
            db.query(Instrument)
            .filter(Instrument.id.in_([p.instrumentid for p in pointings]))
            .all()
        )
