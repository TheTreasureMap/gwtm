...
def get_scale_factor(unit: str) -> float:
    ...
    scale_factors = {"deg": 1.0, "arcmin": 1.0 / 60.0, "arcsec": 1.0 / (60.0 * 60.0)}
    return scale_factors.get(unit, 1.0)


def create_rectangular_footprint(
    height: float, width: float, scale: float
...
def create_circular_footprint(
    radius: float, scale: float, steps: int = 20
) -> List[Tuple[float, float]]:
...
def extract_polygon_coordinates(
    polygon_text: str, scale: float
            ...
            for poly_text in polygons:
                ...
                try:
                    ...
                    if bracket_match:
                        ...
                        vertices, poly_errors = extract_polygon_coordinates(
                            coord_text, scale
                        )
                        if poly_errors:
