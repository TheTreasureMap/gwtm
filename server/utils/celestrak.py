...
def get_data_from_tle(
    datetime_obj: datetime, tle_lat_offset: float = 0, tle_lon_offset: float = 0.21
...
def get_geo_center(
    datetime_obj: datetime, lon: float, lat: float
) -> Tuple[float, float]:
    """
...
def get_earth_sat_pos(
    ...
    try:
        lon, lat, elevation = get_data_from_tle(
            datetime_obj, tle_lat_offset=tle_lat_offset, tle_lon_offset=tle_lon_offset
        )
    except Exception:
        return False, False, False
