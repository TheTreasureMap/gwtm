...
async def post_icecube_notice(
    ...
    existing_notice = (
        db.query(IceCubeNotice)
        .filter(IceCubeNotice.ref_id == notice_data.ref_id)
        .first()
    )
    ...
    events_schemas = [
        IceCubeNoticeCoincEventSchema.model_validate(event) for event in events
    ]

    return {"icecube_notice": notice_schema, "icecube_notice_events": events_schemas}
