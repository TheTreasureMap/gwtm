...
async def test_celestial():
    ...
    return {"message": "Celestial router is working", "astropy_available": ASTROPY_AVAILABLE}
...
async def get_sun_moon_positions(
    time_of_signal: str = Query(..., description="ISO timestamp of the gravitational wave signal")
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to calculate celestial positions: {str(e)}"
        )