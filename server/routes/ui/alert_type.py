...
async def ajax_get_eventcontour(urlid: str, db: Session = Depends(get_db)):
    ...
    if alert.time_coincidence_far != 0 and alert.time_coincidence_far is not None:
        time_coinc_farrate, time_coinc_farunit = get_farrate_farunit(
            alert.time_coincidence_far
        )
        time_coinc_farrate = round(time_coinc_farrate, 2)
        human_time_coinc_far = (
            f"once per {round(time_coinc_farrate, 2)} {time_coinc_farunit}"
        )

    # Format time-sky position coincidence FAR
    human_time_skypos_coinc_far = ""
    if (
        alert.time_sky_position_coincidence_far != 0
        and alert.time_sky_position_coincidence_far is not None
    ):
