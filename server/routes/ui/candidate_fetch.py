...
async def ajax_candidate_fetch(graceid: str, db: Session = Depends(get_db)):
    ...
    candidates = (
        db.query(GWCandidate).filter(GWCandidate.graceid == normalized_graceid).all()
    )
    ...
    if markers:
        payload.append({"name": "Candidates", "color": "", "markers": markers})
