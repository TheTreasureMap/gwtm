...
async def ajax_icecube_notice(graceid: str, db: Session = Depends(get_db)):
    ...
    icecube_notices = (
        db.query(IceCubeNotice).filter(IceCubeNotice.graceid == graceid).all()
    )
    ...
    icecube_notice_events = (
        db.query(IceCubeNoticeCoincEvent)
        .filter(IceCubeNoticeCoincEvent.icecube_notice_id.in_(icecube_notice_ids))
        .all()
    )
