    ...
    for pointing in pointings:
        ...
        if graceid:
            ...
            if alert and alert.time_of_signal and pointing.time:
                ...
                time_diff_hours = (pointing.time - alert.time_of_signal).total_seconds() / 3600
        ...
        if graceid and alert and alert.avgra is not None and alert.avgdec is not None:
            ...
            position_result = db.query(func.ST_AsText(Pointing.position)).filter(
                Pointing.id == pointing.id
            ).first()
