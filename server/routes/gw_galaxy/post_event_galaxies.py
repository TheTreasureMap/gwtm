...
async def post_event_galaxies(
    ...
    try:
        ...
        raise validation_exception(
            ...
            errors=[
                "Timestamp should be in %Y-%m-%dT%H:%M:%S.%f format. e.g. 2019-05-01T12:00:00.00"
            ],
        )
    ...
    if post_doi:
        if request.creators:
            ...
            valid, creators_list = DOIAuthor.construct_creators(
                request.doi_group_id, user.id, db
            )
            if not valid:
                raise validation_exception(
            ...
            creators = [
                {"name": f"{user.firstname} {user.lastname}", "affiliation": ""}
            ]
    ...
    for galaxy_entry in request.galaxies:
        try:
            ...
            if galaxy_entry.position:
                if (
                    all(x in galaxy_entry.position for x in ["POINT", "(", ")", " "])
                    and "," not in galaxy_entry.position
                ):
                ...
                gw_galaxy_entry.position = (
                    f"POINT({galaxy_entry.ra} {galaxy_entry.dec})"
                )
            else:
                errors.append(
                    [
    ...
    if post_doi and valid_galaxies:
        ...
        doi_id, url = create_galaxy_score_doi(
            valid_galaxies, creators, request.reference, graceid, alert.alert_type
        )
