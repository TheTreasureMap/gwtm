...
async def query_alerts(
    ...
    if has_pointings:
        ...
        pointing_subquery = db.query(PointingEvent.graceid).join(
            Pointing, Pointing.id == PointingEvent.pointingid
        ).filter(
        ...
        base_query = base_query.filter(
            GWAlert.graceid.in_(
                db.query(pointing_subquery.c.graceid)
            )
        )
...
async def get_alert_filter_options(db: Session = Depends(get_db)):
    ...
    observing_runs = db.query(GWAlert.observing_run).distinct().filter(
        GWAlert.observing_run.isnot(None)
    ).all()
    observing_runs = sorted([run[0] for run in observing_runs if run[0]])
    
    # Get unique roles
    roles = db.query(GWAlert.role).distinct().filter(
        GWAlert.role.isnot(None)
    ).all()
    roles = sorted([role[0] for role in roles if role[0]])
    
    # Get unique alert types
    alert_types = db.query(GWAlert.alert_type).distinct().filter(
        GWAlert.alert_type.isnot(None)
    ).all()
