...
class GWTMApiService {
    ...
    private setupInterceptors(): void {
        ...
        this.client.interceptors.response.use(
            (response) => {
                if (browser && import.meta.env.DEV) {
                    console.log(`GWTM API Response: ${response.config.method?.toUpperCase()} ${response.config.url}`, response.data);
                }
                return response;
            },
            (error) => {
    ...
    async postEventGalaxies(request: PostEventGalaxiesRequest): Promise<PostEventGalaxiesResponse> {
        return this.post<PostEventGalaxiesResponse, PostEventGalaxiesRequest>('/api/v1/event_galaxies', request);
    }
    ...
    async postIceCubeNotice(request: IceCubeNoticeRequestSchema): Promise<Record<string, any>> {
        return this.post<Record<string, any>, IceCubeNoticeRequestSchema>('/api/v1/post_icecube_notice', request);
    }
    ...
    async getAlertInstrumentsFootprints(graceid?: string, pointing_status?: string, tos_mjd?: number): Promise<any> {
        return this.get<any>('/ajax_alertinstruments_footprints', { graceid, pointing_status, tos_mjd });
    }

    async previewFootprint(ra: number, dec: number, radius?: number, height?: number, width?: number, shape?: string, polygon?: string): Promise<any> {
        return this.get<any>('/ajax_preview_footprint', { ra, dec, radius, height, width, shape, polygon });
    }
    ...
    async spectralRangeFromSelectedBands(band_cov: string, spectral_type: string, spectral_unit: string): Promise<any> {
        return this.get<any>('/ajax_update_spectral_range_from_selected_bands', { band_cov, spectral_type, spectral_unit });
    }
    ...
    async requestDOIAjax(graceid: string, ids?: string, doi_group_id?: string, doi_url?: string): Promise<any> {
