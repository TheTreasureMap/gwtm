...
export const alertService = {
    queryAlerts: async (params?: Record<string, any>): Promise<GWAlertQueryResponse> => {
        ...
        const response = await client.get<GWAlertQueryResponse>('/api/v1/query_alerts', { params: paginatedParams, headers: { 'skip-auth': 'true' } });
    ...
    getInstrumentEventsContributed: async (instrumentId: number): Promise<GWAlertSchema[]> => {
        ...
        const response = await client.get<GWAlertSchema[]>('/api/v1/query_alerts', { params, headers: { 'skip-auth': 'true' } });
        return response.data;
    },

    getAlertFilterOptions: async (): Promise<GWAlertFilterOptionsResponse> => {
        const response = await client.get<GWAlertFilterOptionsResponse>('/api/v1/alert_filter_options', { headers: { 'skip-auth': 'true' } });
        return response.data;
    },
    ...
    getGRBMOCFile: async (graceid: string, instrument: string): Promise<any> => {
        const response = await client.get<any>('/api/v1/grb_moc_file', { params: { graceid, instrument }, headers: { 'skip-auth': 'true' } });
        return response.data;
    },
