<script lang="ts">
    ...
    function handleVisualizationDataLoaded(
        ...
        if (newFootprintData && Array.isArray(newFootprintData) && (minTime === -1 && maxTime === 7)) {
            ...
            if (times.length > 0) {
                ...
                console.log('Calculated time range from footprintData contours:', {minTime, maxTime, timeRange});
            } else {
    ...
    function handleFootprintDataLoaded(event: CustomEvent<{ data: any; timeRange: any }>) {
        ...
        if (timeRangeData) {
            ...
            console.log('No time range data in event, using binding values:', {minTime, maxTime, timeRange});
        }
    }
